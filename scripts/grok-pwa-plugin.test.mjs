...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, {
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-doc-")),
  });
...
test("does not duplicate twitter:card or og:title", () => {
  const ctx = { cwd: mkdtempSync(join(tmpdir(), "grok-og-hello-")) };
...
test("uses the app name in the injected title tag", () => {
  ...
  assert.match(out, /apple-mobile-web-app-title" content="(E\$L Club|East Side Social Club|Wild Race)"/);
});
...
test("renders the manifest with the per-app name", () => {
  ...
  const siteTitle = (() => {
    try {
      return JSON.parse(readFileSync(join(TEMPLATE_ROOT, "src/lib/og/site.json"), "utf8")).title;
    } catch {
      return "";
    }
  })();
  assert.equal(manifest.name, String(siteTitle || "").trim() || "Wild Race");
  assert.equal(manifest.display, "standalone");
  assert.equal(manifest.theme_color, "#070A08");
  assert.ok(manifest.icons.some((icon) => String(icon.src).includes("icon-192.png")));
  assert.ok(manifest.icons.some((icon) => String(icon.src).includes("__grok/icon-180.png")));
});
