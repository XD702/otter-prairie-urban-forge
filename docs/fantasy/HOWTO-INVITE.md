...
2. Friend opens link → enters **real** email → magic link
3. Lands back on `/join?code=EASTSIDE` signed in (Supabase session)
4. Auto `joinLeagueChat` (cap 12)
5. **Team picker** lists exact Yahoo **names** only (never “Team 1–12”):
   - Go birds D**k head  
   - Gibb it to me Baby  
   - Goodwrench  
   - APACHEDOGS  
   ...
   - Cheese me!  
   - WSTD MNGMNT  
6. Claim → RPC `claim_eastside_team` (stable `team_id` under the hood). First claim wins.
7. **Open Chat** → `sessionStorage eastside-open-chat=1` → `/` Chat tab
